 ...
 *(ad_comm::global_datafile) >>  model_name; 
 ...
 projfile_name = cntrlfile_name(1,length(cntrlfile_name)-4) + ".prj";
  Nsr_curves.allocate("Nsr_curves");
 ...
 log_input(retro);
  steepnessprior.allocate(1,Nsr_curves,"steepnessprior");
  cvsteepnessprior.allocate(1,Nsr_curves,"cvsteepnessprior");
  phase_srec.allocate(1,Nsr_curves,"phase_srec");
  sigmarprior.allocate(1,Nsr_curves,"sigmarprior");
  log_sigmarprior.allocate(1,Nsr_curves);
  cvsigmarprior.allocate(1,Nsr_curves,"cvsigmarprior");
  phase_sigmar.allocate(1,Nsr_curves,"phase_sigmar");
 log_input(sigmarprior);
 log_input(cvsigmarprior);
 log_input(phase_sigmar);
  styr_rec_est.allocate(1,Nsr_curves,"styr_rec_est");
  endyr_rec_est.allocate(1,Nsr_curves,"endyr_rec_est");
 log_input(styr_rec_est);
 log_input(endyr_rec_est);
  nrecs_est_shift.allocate(1,Nsr_curves);
  sr_shift.allocate(1,Nsr_curves-1,"sr_shift");
  yy_shift_st.allocate(1,Nsr_curves);
  yy_shift_end.allocate(1,Nsr_curves);
...
catchbiomass_pen= 1./(2*cv_catchbiomass*cv_catchbiomass);
 ...
 phase_Rzero =  4;
 phase_nosr  = -3;
  yy_sr.allocate(styr_sp,endyr+nproj_yrs);
  yy_sr = 1;
  for (i=2;i<=Nsr_curves;i++)
  {
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  mean_log_rec.allocate(1,Nsr_curves,1,"mean_log_rec");
  steepness.allocate(1,Nsr_curves,0.21,Steepness_UB,phase_srec,"steepness");
  log_Rzero.allocate(1,Nsr_curves,phase_Rzero,"log_Rzero");
  rec_dev.allocate(styr_rec,endyr,-15,15,2,"rec_dev");
  log_sigmar.allocate(1,Nsr_curves,phase_sigmar,"log_sigmar");
  m_sigmarsq.allocate(1,Nsr_curves,"m_sigmarsq");
  #ifndef NO_AD_INITIALIZE
    m_sigmarsq.initialize();
  #endif
  m_sigmar.allocate(1,Nsr_curves,"m_sigmar");
  #ifndef NO_AD_INITIALIZE
    m_sigmar.initialize();
  #endif
  sigmarsq.allocate("sigmarsq");
  sigmar.allocate("sigmar");
  alpha.allocate(1,Nsr_curves,"alpha");
  #ifndef NO_AD_INITIALIZE
    alpha.initialize();
  #endif
  beta.allocate(1,Nsr_curves,"beta");
  #ifndef NO_AD_INITIALIZE
    beta.initialize();
  #endif
  ...
  Bzero.initialize();
  #endif
  Rzero.allocate(1,Nsr_curves,"Rzero");
  #ifndef NO_AD_INITIALIZE
    Rzero.initialize();
  #endif
  phizero.allocate(1,Nsr_curves,"phizero");
  #ifndef NO_AD_INITIALIZE
    phizero.initialize();
  #endif
...
dvariable model_parameters::Requil(dvariable& phi, int iyr)
{
  ofstream& mceval= *pad_mceval;
  random_number_generator& rng= *pad_rng;
  RETURN_ARRAYS_INCREMENT();
